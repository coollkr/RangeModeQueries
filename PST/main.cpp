/*
 * It is the main.cpp file to run the pst data structure
 */
#include <iostream>
#include <fstream>
...
void queryExperiment(ApproxMode_PST *approximateModeBkmt, int delta, int** Qa, int* QaSize);
...
int main() {
    ...
    ApproxMode_PST *approximateMode_BKMT = new ApproxMode_PST(array, len, alpha);
...
void queryExperiment(ApproxMode_PST *approximateMode_BKMT, int delta, int** Qa, int* QaSize) {
    int start_index = 0;
    int end_index = 0;
    ...
    ofstream integrated_result("/users/grad/liu1/data_park/PSTMode.txt");
