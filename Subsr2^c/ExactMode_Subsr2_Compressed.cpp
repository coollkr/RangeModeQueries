 ...
 * The abbreviation for this data structure is subsr^2_c,
...
#include "ExactMode_Subsr2_Compressed.h"
#include <iostream>
...
ExactMode_Subsr2_Compressed :: ExactMode_Subsr2_Compressed(int* array, int len, int s) { // constructor
    this->array = array;
    this->len = len;
    this->s = s;

}
ExactMode_Subsr2_Compressed :: ~ExactMode_Subsr2_Compressed() { // destructor
...
void ExactMode_Subsr2_Compressed :: construct() { // construct the data structure
    ...
    if (B0_len != 0 && B1_len != 0) {
        flag_rank_0 = new rank_support_v5<0>(flag);
        flag_rank_1 = new rank_support_v5<1>(flag);
#ifdef MEASURE_SPACE
        space += size_in_bytes(flag[0]); // flag
        space += size_in_bytes(flag_rank_0[0]); // flag_rank_0
        space += size_in_bytes(flag_rank_1[0]); // flag_rank_1
#endif MEASURE_SPACE
    } else {
        flag_rank_1 = nullptr;
        flag_rank_0 = nullptr;
    ...
    if (B1_len != 0) {
        ...
        realBitArray = new rrr_vector<>*[blocks];
        select_0_array = new rrr_vector<>::select_0_type*[blocks];
        select_1_array = new rrr_vector<>::select_1_type*[blocks];
        bit_rank_0_array = new rrr_vector<>::rank_0_type*[blocks];
        bit_rank_1_array = new rrr_vector<>::rank_1_type*[blocks];
        ...
        for (int i = 0; i < blocks; i++) {
            bit_vector* bit = new bit_vector(2 * s + 1, 0);
            int index = 0;
            ...
            bit->bit_resize(index);
            realBitArray[i] = new rrr_vector<>(*bit);



            select_0_array[i] = new rrr_vector<>::select_0_type(realBitArray[i]);
            select_1_array[i] = new rrr_vector<>::select_1_type(realBitArray[i]);
            bit_rank_0_array[i] = new rrr_vector<>::rank_0_type(realBitArray[i]);
            bit_rank_1_array[i] = new rrr_vector<>::rank_1_type(realBitArray[i]);
...
pair<int, int> ExactMode_Subsr2_Compressed :: findMode(int start_index, int end_index) { // find query result, return value: pair<mode, frequency>, parameter: left index of query and right index of query
    pair<int, int> res;
    ...
    if (B0_len == 0) {
        ...
        pair<int, int> resB1 = bitMethod((*flag_rank_1)(start_index - 1) + 1, (*flag_rank_1)(end_index), s , t);
        freq_b1 = resB1.second;
        mode_b1 = resB1.first;
        pair<int, int> resB0 = Third_Method((*flag_rank_0)(start_index - 1) + 1 , (*flag_rank_0)(end_index));
        freq_b0 = resB0.second;
        mode_b0 = resB0.first;
    }
    int final_freq = 0;
...
pair<int, int> ExactMode_Subsr2_Compressed :: Third_Method(int start_index, int end_index) {  // third method part, return value <mode, frequency>, parameter: left index of query and right index of query
    pair<int, int> res;
...
pair<int, int> ExactMode_Subsr2_Compressed :: bitMethod(int start_index, int end_index, int s, int t) { // bit method part, return value <mode, frequency>, parameter: left index of query and right index of query
...
int ExactMode_Subsr2_Compressed :: getCandidateFreq(int len, int freq_c, int x, int i, int j) {  // get the frequency of current candidate, return the frequency of current value -> x in the range from i to j. i: left index, j: right index.
    int currentKey = 0;
...
int ExactMode_Subsr2_Compressed :: getSuffixCandidateFreq(int len, int freq_c, int x, int i, int j) {// get the frequency of current candidate in the suffix , return the frequency of current value -> x in the range from i to j. i: left index, j: right index.
    int currentKey = 0;
...
void ExactMode_Subsr2_Compressed :: update_Ci_Table(int numOfBlocks) {  // update Ci table
...
long long ExactMode_Subsr2_Compressed :: get1DSize (size_t byte, int row) { // calculate the space usage of 1d array, return value is the space, para: size unit, the number of unit
    long long space = 0;
    space = byte * row;
    return space;
}
long long ExactMode_Subsr2_Compressed :: get2DSize (size_t byte, int row, int col) {// calculate the space usage of 2d array, return value is the space, para: size unit, the number of unit in row and col
    long long space = 0;
    space = byte * row * col;
    return space;
}
