/*
 * It is the main.cpp file to run the succ^c data structure
 */
#include <iostream>
...
void queryExperiment(ApproxMode_Succ_Compressed *approximateMode_Rrr_EHMNS, int delta, int** Qa, int* QaSize);
...
int main() {
    ...
    ApproxMode_Succ_Compressed *approximateMode_Rrr_EHMNS= new ApproxMode_Succ_Compressed(array, len, epsilon);
    struct rusage start, end;

    getrusage(RUSAGE_SELF, &start);
    approximateMode_Rrr_EHMNS->construct();
    ...
    queryExperiment(approximateMode_Rrr_EHMNS, delta, Qa, QaSize);


    delete approximateMode_Rrr_EHMNS;
    delete[] array;
    return 0;
}
void queryExperiment(ApproxMode_Succ_Compressed *approximateMode_Rrr_EHMNS, int delta, int** Qa, int* QaSize) {
    ...
    for (int i = 0; i < 200000000; i+=2) {
        ...
        int res = approximateMode_Rrr_EHMNS->findQueryResult(start_index, end_index);
        result[(i / 2)] = res;
    }
    getrusage(RUSAGE_SELF, &end);
