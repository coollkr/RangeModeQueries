 ...
 * The abbreviation for this data structure is sample^c,
...
#include "ApproxMode_Sample_Compressed.h"
#include <iostream>
#include <sdsl/bit_vectors.hpp>
#include <bits/stdc++.h>


long long ApproxMode_Sample_Compressed :: get1DSize (size_t byte, long row) { // calculate the space usage of 1d array, return value is the space, para: size unit, the number of unit
    long long space = 0;
    space = byte * row;
    return space;
}
long long ApproxMode_Sample_Compressed :: get2DSize (size_t byte, long row, long col) { // calculate the space usage of 2d array, return value is the space, para: size unit, the number of unit
    long long space = 0;
    space = byte * row * col;
    return space;
}

ApproxMode_Sample_Compressed :: ApproxMode_Sample_Compressed(int* array, int len, double epsilon) { // constructor
    this->array = array;
    this->len = len;
    this->epsilon = epsilon;
}
int ApproxMode_Sample_Compressed :: construct() { // construct the data structure
    ...
    bitArray = new rrr_vector<>*[k_front_half_bound];
#ifdef MEASURE_SPACE
    ...
    newRank = new rrr_vector<>::rank_1_type*[endK + 1];
    for (int k = 0; k < k_front_half_bound; k++) {
        tmpIndex = 0;
        bit_vector *bit;
        if (mergeSize[k] > 0) {
            bit = new bit_vector(mergeSize[k]);
        } else {
            break;
        }
        ...
        for (int i = 0; i < I_len; i++) { // merge bit for each k
            if (bitSize[k][i] > 0) {
                for (int q = 0; q < bitSize[k][i]; q++) {
                    bit[0][tmpIndex++] = (*tmpbitArray[k][i])[q];
                }
            } else {
                break;
            }
        }
        bitArray[k] = new rrr_vector<>(*bit);
        if (mergeSize[k] > 0) {  // update rank array
            newRank[k] = new rrr_vector<>::rank_1_type(bitArray[k]);
#ifdef MEASURE_SPACE
...
ApproxMode_Sample_Compressed :: ~ApproxMode_Sample_Compressed() { // destructor
    delete[] saveK;
    delete[] largeK;
    for (int i = 0; i < I_len; i++) {
        delete [] Sl[i];
...
int ApproxMode_Sample_Compressed :: findQueryResult(int start_index, int end_index) { // return the mode, parameter: left index in the query and right index in the query
...
void ApproxMode_Sample_Compressed :: getSaveK() { // update saveK array
...
void ApproxMode_Sample_Compressed :: getLargeK() { // update saveK array
...
pair<int, int> ApproxMode_Sample_Compressed :: findTi(int i, int k) { // get the value in original Ti table based on i and k
...
int ApproxMode_Sample_Compressed :: successor_search(int i, int j) { // binary search in Qi table
...
int ApproxMode_Sample_Compressed :: resultBinarySearch(int left, int right) { // binary search in Ti table
