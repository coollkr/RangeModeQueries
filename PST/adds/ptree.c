...
void
p_tree_replace_node (PTree    *tree,
                     gpointer  key,
                     gpointer  value, guint version)
{
    g_return_if_fail (tree != NULL);
    //p_tree_insert_internal (tree, key, value, TRUE);
    PTreeNode* node = p_tree_find_node(tree, key, P_TREE_SEARCH_EXACT , version);
    node->data->value = value;
    node->data->key = key;
#ifdef P_TREE_DEBUG
    {
...
gpointer
p_tree_lookup_related_nv (PTree          *tree,
                          guint           version,
                          gconstpointer   key,
                          PTreeSearchType search_type)
{
    ...
    return node ? node->nv : NULL;
}
gpointer
p_tree_lookup_related_v_key (PTree          *tree,
                             guint           version,
                             gconstpointer   key,
                             PTreeSearchType search_type)
{
