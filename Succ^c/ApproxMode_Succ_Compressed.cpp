 ...
 * The abbreviation for this data structure is succ^c,
...
#include "ApproxMode_Succ_Compressed.h"
#include <iostream>
...
ApproxMode_Succ_Compressed :: ApproxMode_Succ_Compressed(int* array, int len, double epsilon) { // constructor
    this->array = array;
    this->len = len;
    this->epsilon = epsilon;
}

void ApproxMode_Succ_Compressed :: construct() { // construct the data structure
    ...
    Qk = new rrr_vector<>*[Qk_len];
    Qk_select = new rrr_vector<>::select_0_type*[Qk_len];
    Qk_rank_1 = new rrr_vector<>::rank_1_type*[Qk_len];
    numberOfZeroInQk = new int[Qk_len];

#ifdef MEASURE_SPACE
    space += (Qk_len * sizeof(rrr_vector<>*));  // Qk pointer
    space += (Qk_len * sizeof(rrr_vector<>::select_0_type*)); // Qk_select pointer
    space += (Qk_len * sizeof(rrr_vector<>::rank_1_type*)); // Qk_rank_1 pointer
    space += (Qk_len * sizeof(int)); // numberOfZeroInQk
#endif MEASURE_SPACE

    for (int k = 0; k < Qk_len; k++) {
        bit_vector *bit = new bit_vector(bitSize[k]);
        ...
        Qk[k] = new rrr_vector<>(*bit);
        Qk_select[k] = new rrr_vector<>::select_0_type(Qk[k]);
        Qk_rank_1[k] = new rrr_vector<>::rank_1_type(Qk[k]);
    ...
    Kr = new rrr_vector<>**[k_upper_bound];
    for (int i = 0; i < k_upper_bound; i++) {
        Kr[i] = new rrr_vector<>*[2];
    }
    Kr_select = new rrr_vector<>::select_1_type**[k_upper_bound];
    for (int i = 0; i < k_upper_bound; i++) {
        Kr_select[i] = new rrr_vector<>::select_1_type*[2];
    }
    Kr_rank_0 = new rrr_vector<>::rank_0_type**[k_upper_bound];
    for (int i = 0; i < k_upper_bound; i++) {
        Kr_rank_0 [i] = new rrr_vector<>::rank_0_type*[2];
    }
...
#ifdef MEASURE_SPACE
    space += (k_upper_bound * sizeof(rrr_vector<>*));  // Kr pointer
    space += (k_upper_bound * sizeof(rrr_vector<>::select_1_type*)); // Kr select pointer
    space += (k_upper_bound * sizeof(rrr_vector<>::rank_0_type*)); // Kr rank 0 pointer
    ...
    for (int k = 0; k < k_upper_bound; k++) {
        ...
        bit_vector *bitOne =  new bit_vector(len + len / ceil_f_2k_minus_1 + 1);
        ...
        (*bitOne).resize(lastOne);
        numberOfOneInKr[k][0] = count;


        Kr[k][0] = new rrr_vector<>(*bitOne);

        Kr_select[k][0] = new rrr_vector<>::select_1_type(Kr[k][0]);
        Kr_rank_0[k][0] = new rrr_vector<>::rank_0_type (Kr[k][0]);
        ...
        bit_vector *bitTwo = new bit_vector(len + len / ceil_f_2k + 1);
        ...
        (*bitTwo).resize(lastOne);
        Kr[k][1] = new rrr_vector<>(*bitTwo);


        Kr_select[k][1] = new rrr_vector<>::select_1_type(Kr[k][1]);
        Kr_rank_0[k][1] = new rrr_vector<>::rank_0_type(Kr[k][1]);
...
int ApproxMode_Succ_Compressed :: getIndexValue(int index) { // return the value of array[i]
    return array[index];
}
int ApproxMode_Succ_Compressed :: findQueryResult(int start_index, int end_index) {  // return the mode parameter: left index in the query and right index in the query
...
ApproxMode_Succ_Compressed :: ~ApproxMode_Succ_Compressed() { // destructor
...
int ApproxMode_Succ_Compressed :: findLargeMode(int start, int end) {  // binary search in large frequency, parameter: left index in the query and right index in the query
...
int ApproxMode_Succ_Compressed :: getRi(int curK, int i){ // get the ri based on k and si
    int res = len;
...
int ApproxMode_Succ_Compressed :: getRj(int curK, int j){ // get the rj based on k and sj
    int res = len;
...
inline int ApproxMode_Succ_Compressed :: findSi(int start, int k) {// return si value based on current left index and k
    // get the si value based on k and left index
    return start / ceil_f_2k_minus_1Array[k];
}

inline int ApproxMode_Succ_Compressed :: findSj(int start, int k) { // get the sj value based on k and left index
    return start / ceil_f_2kArray[k];
}

int ApproxMode_Succ_Compressed :: findLowFreq(int start, int end ) { // binary search in the Qk
...
int ApproxMode_Succ_Compressed :: getQki(int k, int i) { // get the value in the Qk table based on k and left index
