/*
 * It is the main.cpp file to run the sqrt data structure
 */
...
void queryExperiment(ExactMode_Sqrt *exactMode_CDLMW);
...
int main() {
    ...
    ExactMode_Sqrt *exactMode_CDLMW = new ExactMode_Sqrt(array, len, s);


    struct rusage start, end;

    getrusage(RUSAGE_SELF, &start);
    exactMode_CDLMW->construct();
    ...
    queryExperiment(exactMode_CDLMW);


    delete exactMode_CDLMW;
    delete[] array;
    return 0;
}
void queryExperiment(ExactMode_Sqrt *exactMode_CDLMW) {
    ...
    for (int i = 0; i < 2000000; i+=2) {
        ...
        pair<int, int> res = exactMode_CDLMW->findMode(start_index, end_index);
        mode = res.first;
        freq = res.second;
        result[(i / 2)] = freq;
    }
