/*
 * It is the main.cpp file to run the subsr^1_c data structure
 */
#include <iostream>
#include <cmath>
...
void queryExperiment(ExactMode_Subsr1_Compressed *exactMode_Rrr_CDLMW);
...
int main() {
    ...
    ExactMode_Subsr1_Compressed *exactMode_Rrr_CDLMW = new ExactMode_Subsr1_Compressed(array, len, s);


    struct rusage start, end;

    getrusage(RUSAGE_SELF, &start);
    exactMode_Rrr_CDLMW->construct();
    ...
    queryExperiment(exactMode_Rrr_CDLMW);

    delete[] array;
    delete exactMode_Rrr_CDLMW;
...
void queryExperiment(ExactMode_Subsr1_Compressed *exactMode_Rrr_CDLMW) {
    ...
    for (int i = 0; i < 2000000; i+=2) {
        ...
        pair<int, int> res = exactMode_Rrr_CDLMW->findMode(start_index, end_index);
        mode = res.first;
        freq = res.second;
        result[(i / 2)] = freq;
    }
    ...
    ofstream integrated_result("/users/grad/liu1/data_ip/TimothyFreq_book_1000.txt");
