/*
 * It is the main.cpp file to run the sample^c data structure
 */
#include <iostream>
...
void queryExperiment(ApproxMode_Sample_Compressed *approximateMode_Bit_GJLT, int delta, int** Qa, int* QaSize);
...
int main() {
    ...
    cout << "epsilonPrime: " << epsilonPrime << endl;




    ApproxMode_Sample_Compressed *approximateMode_Rrr_GJLT = new ApproxMode_Sample_Compressed(array, len, epsilonPrime);
    struct rusage start, end;

    getrusage(RUSAGE_SELF, &start);
    int t = approximateMode_Rrr_GJLT->construct();
    ...
    queryExperiment(approximateMode_Rrr_GJLT, delta, Qa, QaSize);


    delete approximateMode_Rrr_GJLT;
    delete[] array;
    return 0;
}
void queryExperiment(ApproxMode_Sample_Compressed *approximateMode_Bit_GJLT, int delta, int** Qa, int* QaSize) {
    ...
    for (int i = 0; i < 200000000; i+=2) {
        ...
        int res = approximateMode_Bit_GJLT->findQueryResult(start_index, end_index);
        result[(i / 2)] = res;
    }
    getrusage(RUSAGE_SELF, &end);
    ...
    cout << endl;
    ofstream moderesult("/users/grad/liu1/data_park/1plusMode.txt");
    //ofstream moderesult("/users/grad/liu1/1plusMode.txt");
